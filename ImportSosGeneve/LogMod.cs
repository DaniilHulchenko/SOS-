...
namespace ImportSosGeneve
{
    class LogMod
    {
        ...
        public void EcrireLog(string type, string texte)
        {
            if (objLog == null)
            {
                //string chemin_logs = ConfigurationManager.AppSettings["Path_Log_Postfinance"];
                string chemin_logs = SosMedecins.SmartRapport.Systeme.OutilsExt.ParamAppli.Path_Log_Postfinance;
                                                                       
                objLog = new log4Raid( @chemin_logs, "Log", 0, "");                               
            }
            objLog.EcrireLog(type, texte);
        }


        public void EcrireLogTA(string type, string texte)
        {
            if (objLog == null)
            {
                //string chemin_logs = ConfigurationManager.AppSettings["Path_Log_Postfinance"];
                ...
                objLog = new log4Raid(@chemin_logs, "TaLog", 0, "");
            }
            objLog.EcrireLog(type, texte);
        }
 ...
 string p_date_format)
            ...
            public void EcrireLog(string type, string texte)
            {
                try
                {
                    ...
                    LogStream.WriteLine(type + " : " + texte);
